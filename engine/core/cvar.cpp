...
namespace cyb
{
    static CVarRegistryMapType g_cvarRegistry;

    // This registry is for pre RegisterStaticCVars() only.
    static std::vector<CVarBase*>& StaticRegistry()
    {
        static std::vector<CVarBase*> registry;
        return registry;
    }
    ...
    static void RegisterCVar(CVarBase* cvar)
    {
        assert(cvar != nullptr);
